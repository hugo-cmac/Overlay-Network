    	...
    	if (packet.get() > 0){
	        ...
	        switch(result){
		        ...
		        case 1:{//new exitSocket
		            ...
		            // }else{
					// 	packet.buidResponse(streamID, r.first, false);
		            // }
					packet.forward();
		            break;
		        }
...
void proxyLocalHandler(Protocol packet, int streamID){
	byte* buffer = (byte*)malloc(PACKET);
	int n = 1;
	...
	while(n){
		...
		if (n > 0){
			packet.build(streamID, path[streamID], true, buffer);
...
void proxyServerProcedure(int proxyClient){
    ...
    if (client.handleGreetingPacket()) {
    	...
    	if (client.handleConnectionReq()) { // se for dominio ou ipv4 retorna true
			...
			if (streamID != -1){
	    		packet.freshBuild(type, streamID, addr);
				packet.forward();
