...
class FIFO{
	private:
		...
		f* head = NULL;
	public:
		void push(int streamID){
			f* n = (f*)malloc(sizeof(f));
			n->streamID = streamID;
			n->next = NULL;
			if (head == NULL){
				head = n;
...
//Thread control
pthread_t localSlave[MAXFDS];														//
pthread_mutex_t localLock = PTHREAD_MUTEX_INITIALIZER;								//
pthread_cond_t localWait = PTHREAD_COND_INITIALIZER;								//
FIFO localfifo;																	    //
//////////////////////////////////////////////////////////////////////////////////////
	    	...
	    	if (clientsock.responsePacket() > 0) {
	    		...
	    		if (streamID >= 0){
	    			...
	    			if(exitSocket > 0){
						...
						localfifo.push(streamID);
						pthread_cond_signal(&localWait);
						pthread_mutex_unlock(&localLock);

						pthread_mutex_lock(&exitLock);
						exitfifo.push(streamID);
...
void closeStream(short streamID){
...
void closeExitStream(int streamID){
	close(exitSocks[streamID]);
	exitSocks[streamID] = -1;
...
void* exitthreadPool(void* arg){
	int streamID;
	while (1){
		pthread_mutex_lock(&exitLock);

		if ((streamID = exitfifo.pop()) == -1){
			pthread_cond_wait(&exitWait, &exitLock);

			streamID = exitfifo.pop();
	    }
	    pthread_mutex_unlock(&exitLock);

	    if (streamID != -1){
	    	exitNodeHandler(streamID);
	    }
	}
}

void closeLocalStream(int streamID){
	close(localSocks[streamID]);
	localSocks[streamID] = -1;
...
void* localthreadPool(void* arg){
	int streamID;
	while (1){
		pthread_mutex_lock(&localLock);
		if ((streamID = localfifo.pop()) == -1){
			pthread_cond_wait(&localWait, &localLock);

			streamID = localfifo.pop();
		}
	    pthread_mutex_unlock(&localLock);

	    if (streamID != -1){
	    	proxyLocalHandler(streamID);
	    }
	}
}
...
void setupTerminal(){
	...
	for (int i = 0; i < MAXFDS; ++i){
		...
		pthread_create(&localSlave[i], NULL, localthreadPool, NULL);
		pthread_detach(localSlave[i]);
		//printLocalThread(i, 1);

		pthread_create(&exitSlave[i], NULL, exitthreadPool, NULL);
		pthread_detach(exitSlave[i]);
		//printLocalThread(i, 1);
	}
}
