...
void getDomain(char* addr, unsigned char* packet){
	int i = 0;
    while (i < (255-8)) {
        if(packet[i] == 0){
            memcpy(addr, packet+i+1, 255-9-i);
			return;
        }else{
            i++;
        }
...
int webConnect(byte* socksPacket){
	...
	if(socksPacket[0] == 0x04){
		...
		printf("%d %d %d %d \n", socksPacket[4], socksPacket[5], socksPacket[6], socksPacket[7]);
		if ( !socksPacket[4] &&  !socksPacket[5] && !socksPacket[6] && socksPacket[7]){
			memset(addr, '\0' , 255);
			getDomain(addr, &socksPacket[8]);
			printf("%s\n", addr);
